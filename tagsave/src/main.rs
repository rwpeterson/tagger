...
async fn main() -> Result<()> {
    ...
    let settings: Option<InputState>;
...

    {
        ...
        let (respond_to_settings, response_settings) = flume::bounded(1);
        let _ = client.settings_sender.send(ClientMessage::GetSettings { respond_to: respond_to_settings });

        settings = Some(response_settings.recv()?);
    } // drop client handle
    ...
    for channel in CHAN16 {
        record.channel_settings.push(
            cfg::ChannelSettings {
                channel,
                invert: Some(bit::mask_to_chans(settings.as_ref().unwrap().inversion_mask).contains(&channel)),
                delay: Some(settings.as_ref().unwrap().delays[channel as usize - 1]),
                threshold: Some(settings.as_ref().unwrap().thresholds[channel as usize - 1]),
            }
        );
    }
